const fetchFavoriteGames = async (data) => {
  ...
  listVideoContainer.innerHTML =
    `<div style="display:flex; flex-direction:column; align-items:center; justify-content; center; margin-top: 350px; width:100%; height:100%; font-size:12pt; color:yellow;">
        <div class="spinner-border"></div>
          Loading please wait...
          <div class="lds-dual-ring"></div>
    </div>`;

  const fetchedGames = await fetch('userFetchFavoriteGames.php',
    {
      method: "POST",
  ...
  const response = fetchedGames['response'] !== null && fetchedGames['response'] !== undefined;
  if (response) {
    alert(fetchedGames['response']);
    return;
  }
  console.log(JSON.stringify(fetchedGames['response']));
  let found = fetchedGames['games'] !== null && fetchedGames['games'] !== undefined && fetchedGames['games'].length !== 0;
  ...
  if (!found) {
    ...
    for (i; i <= maxIndex; i++) {
      ...
      eyeButton.innerHTML =
        `<div id="eye-div-${index}" style="display:flex; justify-content:center;">
      ...
      starButton.innerHTML =
        `<div id="star-div-${index}" style="display:flex; justify-content:center;">
...
const toggleFavorite = () => {
  ...
  document.getElementById('toggle-state').innerHTML = toggled ? 'false' : 'true';
...
const clickGame = (e) => {
  ...
  console.log("target Node Name : " + e.target.nodeName + ' ' + e.target.getAttribute('id'));
  ...
  if (isEyeDiv) {
    ...
    let favorite_state = document.getElementById("favorite-state-"+ favorite_id);
    ...
    const updateGameFavoriteStatus = async (data) => {
      ...
      const updateFavoriteGameStatus = await fetch('userUpdateFavoriteGame.php',
        {
          method: "POST",
          body: JSON.stringify(data),
          headers: {
...
const userFetchGames = async () => {

  let email = document.getElementById('email-user').innerHTML;
  let select_delivery_date = document.getElementById("select-delivery-date");
  let delivery_date = "";
  if (!isNullOrUndefined(select_delivery_date) && select_delivery_date.selectedIndex !== 0) {
    delivery_date = select_delivery_date.options[select_delivery_date.selectedIndex].text;
  ...
  let type = "";
  if (!isNullOrUndefined(select_category) && select_category.selectedIndex !== 0) {
  ...
  const data = { 'email': email, 'type': type, 'device': media, 'delivery_date': delivery_date, 'status': status, 'weight': weight, 'engine': engine, 'fav': fav };
  ...
  const fetchedGames = await fetch('userFetchVideoGames.php',
    {
      method: "POST",
      body: JSON.stringify(data),
      headers: {
  ...
  let nodata = fetchedGames['no_data'] != null && fetchedGames['no_data'] != undefined;
  let dataFound = fetchedGames['games'] != null && fetchedGames['games'] != undefined;
