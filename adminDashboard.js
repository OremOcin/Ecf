...
const fetchGames = async ()=>{
  ...
  const fetchedGames = await fetch('adminBrowseVideoGames.php', 
  {
      method: "POST",
  ...
  console.log("no data = "+ nodata + " Data found = "+ dataFound);
      ...
      let content = 
         `<span id="hidden-variables#${index}" hidden>
...
const fetchBudgets = () => {
  ...
  const fetchedBudgets = await fetch('fetchGameBudgets.php', 
  {
      method: "POST",
      body: JSON.stringify(data),
      headers: {
  ...
  const clickTrash = async (e) => {
    ...
    const fetchRemoveBudget = await fetch('removeBudget.php', 
    {
        method: "POST",
        body: JSON.stringify(data),
        headers: {
    ...
    }).catch( (error)=>{console.log("CreateGame error doFetch "+error);} ).
    then( (result) =>{
       return  result.json();
  ...
  const updateBudgets = async (e) => {
    ...
    const fetchedBudgets = await fetch('fetchAddBudget.php', 
    {
        method: "POST",
  ...
  for(let budget of fetchedBudgets['response'])
  {
    ...
    <div id="trash-button-motif" hidden>${budget['comments']}</div>
      <svg id=${"button-trash-svg"+index} xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-trash-fill" viewBox="0 0 16 16">
        <path id=${"button-trash-svg-path-"+index} d="M2.5 1a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1H3v9a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V4h.5a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H10a1 1 0 0 0-1-1H7a1 1 0 0 0-1 1H2.5zm3 4a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 .5-.5zM8 5a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7A.5.5 0 0 1 8 5zm3 .5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 1 0z"/>
      </svg>
      </button>
    </div>
      <div id=${"budget-"+index} style="width:90%; height:100%;" >
          <li id=${"budget-li-"+index} class="budgetItem" >
            <label id=${"budget-li-label-0-"+index} for=${budgetValue}>Montant</label>
            <input id=${budgetValue} type="text" value=${budget['value']} readonly></input>
            <label id=${"budget-li-label-1-"+index} for=${budgetDate}>Date</label>
            <input id=${budgetDate} type="text" value=${budget['date']} readonly></input>
            <label id=${"budget-li-label-1-"+index} for=${budgetMotif}>Motif</label>
    ...
    document.getElementById(trashButtonId).addEventListener("click", clickTrash);
    index++;
  }
  }
