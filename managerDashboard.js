...
async function fetchNews(data) {
  ...
  }).catch( (error)=>{console.log("publishNews error doFetch "+error);} ).
  then( (result) =>{
     return  result.json();
  });
...
async function fetchAllNews() {
  ...
  }).catch( (error)=>{console.log("fetchAllNews error doFetch "+error);} ).
  then( (result) =>{
     return  result.json();
  });
  ...
  if(allNews === null || allNews === undefined || allNews.length === 0) {
    ...
    newsContainer.innerHTML = `<div style="display:flex; flex-direction:column; align-items:center; justify-content:center; width:100%; height:100%; font-size:15pt; color:yellow;" >
             ...
             ${isNullOrUndefined ? `<p>Un problème est survenu avec la base de donnée.</p>` : `<p>La base des news est vide.</p>`} 
         </div>
         `;
         return;
  } 
  ...
  let i = 0;
       ...
       `<div id="videoToast-${i}" class="toast" >
          <div id="toast-header-${i}" class="toast-header">
              <div id="toast-header-title-${i}" class="toast-header-title">${allNews[i]['title']}</div>
              <div id="toast-header-date-${i}" class="toast-header-date">
                <div>${allNews[i]['date']}</div>
              </div>
          </div>
          <div id="toast-contents-${i}" class="toast-contents">
                ...
                <div class="toast-image">
                  <img id="screenshot#_${i}" style="max-width: 100%;box-shadow: 0 0 15px black;border-radius:2px; " src="${allNews[i]['image']}" alt="Girl in a jacket"></img>
                </div>
          </div>
        </div>`;
          if (i !== size - 1) {
